<?php 
    ...
    function get_all_payment_customer($idCustomer){
        global $conn;
    ...
    function get_limit_payment_customer($limit, $offset, $idCustomer) {
        global $conn;
        $result = mysqli_query($conn, "SELECT p.*, o.order_number FROM payments p JOIN orders o ON o.id = p.order_id WHERE user_id = $idCustomer ORDER BY p.payment_date DESC LIMIT $offset,$limit ");
        $payments = [];
        while ($row = mysqli_fetch_assoc($result)) {
            $payments[] = $row;
        }
